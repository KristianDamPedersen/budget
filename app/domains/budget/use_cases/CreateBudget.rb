module Budget
  module UseCases
    class CreateBudget
    Request = Data.define(:name, :owned_by, :categories)
      Response = Data.define(:name, :owned_by, :budget_categories)

      def call(request)
        budget = nil
        ActiveRecord::Base.transaction do
          budget = Budget::Domain::Budget.create!(name: request.name, owned_by: request.owned_by)
          p request.categories
          request.categories.each do |_, category|
            budget.budget_categories.create(name: category["name"])
            end
        end
      Response.new(
          name: budget.name,
          owned_by: budget.owned_by,
          budget_categories: budget.budget_categories
        )
      end
    end
  end
end
